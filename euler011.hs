...
maxProduct len table = maximum $ map maxDirProduct [(1,-1), (1,0), (1,1), (0,1)]
    where maxDirProduct (dX,dY) = maximum [prod dX dY x y | x <- indexRange dX, y <- indexRange dY]
          prod dX dY x y = product [table ! (y+dY*i,x+dX*i) | i <- [0..len-1]]
          indexRange delta = [mini..maxi]
